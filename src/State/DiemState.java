...
public class DiemState {
    ...
    // read
    public ArrayList<DiemModel> view() {
        try {
            ArrayList<DiemModel> diemList = new ArrayList<>();

            String insertquery = "SELECT ketqua.masv,ketqua.mamon,`hosv`,`tensv`,`diem`,`tenmon` FROM `ketqua` JOIN sinhvien on sinhvien.masv = ketqua.masv JOIN mon on ketqua.mamon = mon.ma";
    ...
    public ArrayList<DiemModel> SearchText(String s) {
        try {
            ArrayList<DiemModel> diemList = new ArrayList<>();
            String insertquery = String
                    .format("SELECT ketqua.masv,ketqua.mamon,`hosv`,`tensv`,`diem`,`tenmon` FROM `ketqua` JOIN sinhvien on sinhvien.masv = ketqua.masv JOIN mon on ketqua.mamon = mon.ma WHERE locate('%s',tensv)",
                            s);
            ResultSet result = statement.executeQuery(insertquery);
